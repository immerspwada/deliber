-- =====================================================
-- Migration: 176_fix_admin_dashboard_functions.sql
-- Feature: Admin Dashboard Data Display Fix
-- Description: Fix all admin RPC functions to handle missing tables gracefully
-- =====================================================
...
DROP FUNCTION IF EXISTS get_all_providers_for_admin(TEXT, TEXT, INT, INT);
DROP FUNCTION IF EXISTS count_providers_for_admin(TEXT, TEXT);
...
-- =====================================================
-- 7. SEED DEMO DATA (if tables are empty)
-- =====================================================
...
BEGIN
  ...
  IF v_user_count = 0 THEN
    ...
    INSERT INTO users (id, email, first_name, last_name, phone_number, member_uid, verification_status, created_at)
    VALUES (
      gen_random_uuid(),
      'customer@demo.com',
      'Demo',
  ...
  ELSE
    RAISE NOTICE 'Users already exist, skipping demo data';
  END IF;
END $$;

-- =====================================================
-- 8. VERIFY FUNCTIONS WORK
-- =====================================================
DO $$
DECLARE
  v_stats RECORD;
...
BEGIN
  ...
  SELECT * INTO v_stats FROM get_admin_dashboard_stats();
  RAISE NOTICE 'Dashboard Stats: users=%, providers=%, rides=%', v_stats.total_users, v_stats.total_providers, v_stats.total_rides;
  
  -- Test order count
  SELECT count_all_orders_for_admin(NULL, NULL) INTO v_order_count;
  RAISE NOTICE 'Total Orders: %', v_order_count;
  
  -- Test provider count
  SELECT count_providers_for_admin(NULL, NULL) INTO v_provider_count;
  RAISE NOTICE 'Total Providers: %', v_provider_count;
  
  RAISE NOTICE 'All functions verified successfully!';
END $$;
