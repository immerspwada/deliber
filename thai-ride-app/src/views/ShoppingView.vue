...
const { calculateDistance, currentLocation, getCurrentPosition } = useLocation()
...
const hasRoute = computed(() => !!(storeLocation.value && deliveryLocation.value))

const canSubmit = computed(() => 
  storeLocation.value && deliveryLocation.value && itemList.value.trim() && budgetLimit.value
)

const hasEnteredData = computed(() => {
  return storeAddress.value || deliveryAddress.value || 
         itemList.value || specialInstructions.value ||
...
const handleTouchEnd = (e: TouchEvent) => {
  ...
  swipeOffset.value = 0
  setTimeout(() => { stepDirection.value = null }, 300)
}
...
const handleSwipeNext = () => {
  ...
  } else if (currentStep.value === 'items' && itemList.value.trim() && budgetLimit.value) {
    triggerHaptic('medium')
...
const handleMapPickerConfirm = (location: GeoLocation, type: 'store' | 'delivery') => {
  triggerHaptic('heavy')
  if (type === 'store') {
...
const handleRouteCalculated = (data: { distance: number; duration: number }) => {
  ...
  serviceFee.value = calculateServiceFee(parseFloat(budgetLimit.value) || 0, data.distance)
}
...
const handleSubmit = async () => {
  if (!canSubmit.value || !storeLocation.value || !deliveryLocation.value) return
...
<template>
  ...
  >
    ...
    <!-- Bottom Panel -->
    <div class="bottom-panel" :class="{ expanded: currentStep === 'items' || currentStep === 'confirm' }">
        ...
        >
          ...
          <div class="quick-actions-grid">
            ...
            <button 
              ...
              @touchend="handleButtonRelease"
              @click="showStoreMapPicker = true; triggerHaptic('light')"
            >
          ...
          <Transition name="scale-fade">
            <div v-if="storeLocation" class="selected-location-card success">
              ...
              <div class="location-info">
                ...
                <span class="location-label">{{ storeName || storeAddress }}</span>
              </div>
          ...
          <Transition name="slide-up">
            <button 
              ...
              @click="currentStep = 'delivery'; triggerHaptic('medium')"
            >
      ...
      <template v-else-if="currentStep === 'delivery'">
        ...
        >
          ...
          <div class="route-preview-card">
            <div class="route-preview-item">
              ...
              <div class="route-preview-text">
                ...
                <span class="route-preview-value">{{ storeName || storeAddress }}</span>
              </div>
              <button class="edit-btn" @click="currentStep = 'store'; triggerHaptic('light')">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <button 
            ...
            @click="showDeliveryMapPicker = true; triggerHaptic('light')"
          >
          ...
          <!-- Route Info -->
          <Transition name="scale-fade">
            <div v-if="deliveryLocation && estimatedDistance > 0" class="route-info-card">
          ...
          <Transition name="slide-up">
            <button 
              ...
              class="continue-btn primary"
              @click="currentStep = 'items'; triggerHaptic('medium')"
            >
      ...
      <template v-else-if="currentStep === 'confirm'">
        <div class="step-content animate-step scrollable">
          ...
          <div class="summary-card">
            ...
            <div class="summary-section">
              <div class="summary-label">รายการสินค้า ({{ itemCount }} รายการ)</div>
              <div class="items-preview">
                <div v-for="(item, index) in itemList.split('\n').filter(l => l.trim()).slice(0, 5)" :key="index" class="item-line">
            ...
            <div v-if="images.length > 0" class="summary-section">
              ...
              <div class="images-preview">
                <img v-for="img in images" :key="img.id" :src="img.preview" alt="Reference" />
              </div>
            </div>
