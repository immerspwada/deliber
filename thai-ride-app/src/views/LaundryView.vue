...
const serviceTypes = [
  { id: 'wash-fold' as LaundryService, name: 'ซัก-พับ', desc: 'ซักและพับเรียบร้อย', price: '฿40/กก.', icon: 'wash' },
  { id: 'wash-iron' as LaundryService, name: 'ซัก-รีด', desc: 'ซักและรีดเรียบ', price: '฿60/กก.', icon: 'iron' },
  { id: 'dry-clean' as LaundryService, name: 'ซักแห้ง', desc: 'สำหรับเสื้อผ้าพิเศษ', price: '฿150/ชิ้น', icon: 'dry' },
  { id: 'express' as LaundryService, name: 'ด่วน 6 ชม.', desc: 'รับคืนภายใน 6 ชั่วโมง', price: '+฿100', icon: 'express' }
]
...
const toggleService = (id: LaundryService) => {
  ...
  if (index === -1) {
    ...
    if (['wash-fold', 'wash-iron', 'dry-clean'].includes(id)) {
      selectedServices.value = selectedServices.value.filter(s => !['wash-fold', 'wash-iron', 'dry-clean'].includes(s))
    }
...
const isFormValid = computed(() => {
  ...
  const hasMainService = selectedServices.value.some(s => ['wash-fold', 'wash-iron', 'dry-clean'].includes(s))
...
const submitRequest = async () => {
  ...
  const scheduledPickup = new Date(`${pickupDate.value}T${pickupTime.value}`).toISOString()
...
const selectedServicesNames = computed(() => {
  return selectedServices.value.map(s => {
  ...
  <div class="laundry-page">
    ...
    <div class="content">
      ...
      <section class="section">
        <h2 class="section-title">เลือกบริการ <span class="required">*</span></h2>
        <p class="section-hint">เลือกบริการหลัก 1 อย่าง และเพิ่มตัวเลือกด่วนได้</p>
        <div class="service-grid">
          ...
          >
            <div class="service-icon">
              <svg v-if="service.icon === 'wash'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <rect x="3" y="2" width="18" height="20" rx="2"/>
                <circle cx="12" cy="13" r="5"/>
                <path d="M9 13c0-1.5 1.5-2 3-1s3 .5 3-1"/>
                <circle cx="7" cy="6" r="1" fill="currentColor"/>
              </svg>
              <svg v-else-if="service.icon === 'iron'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M3 17h18l-3-8H6l-3 8z"/>
                <path d="M6 9V5a2 2 0 012-2h8a2 2 0 012 2v4"/>
                <path d="M8 17v4M16 17v4"/>
              </svg>
              <svg v-else-if="service.icon === 'dry'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <!-- Pickup Address -->
      <section class="section">
        <h2 class="section-title">ที่อยู่รับผ้า <span class="required">*</span></h2>
      ...
      <!-- Pickup Schedule -->
      <section class="section">
        <h2 class="section-title">วันเวลารับผ้า <span class="required">*</span></h2>
      ...
      <div v-if="selectedServices.length > 0" class="price-estimate">
        ...
        <p class="price-note">* คำนวณจากน้ำหนัก {{ estimatedWeight || 3 }} กก. ราคาจริงขึ้นอยู่กับน้ำหนักจริง</p>
      </div>
    </div>
