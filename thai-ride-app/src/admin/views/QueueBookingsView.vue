...
async function loadData() {
  const result = await api.getQueueBookings(filters.value, {
...
onMounted(() => {
  uiStore.setBreadcrumbs([{ label: "Queue Bookings" }]);
  loadData();

  // Setup realtime for queue_bookings
  realtime.subscribe({
    tables: ["queue_bookings"],
    onChange: (table, eventType) => {
      console.log(`[QueueBookingsView] Realtime: ${table} ${eventType}`);
      loadData();
      uiStore.showInfo(
