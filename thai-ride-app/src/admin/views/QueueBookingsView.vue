...
const serviceType: ServiceType = 'queue'
...
async function loadData() {
  const result = await api.getQueueBookings(filters.value, { page: currentPage.value, limit: pageSize.value })
  ...
  <div class="queue-view">
    <div class="page-header">
      <div class="header-left">
        <h1 class="page-title">Queue Bookings</h1>
    ...
    <div class="status-tabs">
      ...
      <button class="status-tab" :class="{ active: statusFilter === 'pending' }" @click="statusFilter = 'pending'">รอยืนยัน</button>
      <button class="status-tab" :class="{ active: statusFilter === 'confirmed' }" @click="statusFilter = 'confirmed'">ยืนยันแล้ว</button>
      <button class="status-tab" :class="{ active: statusFilter === 'in_progress' }" @click="statusFilter = 'in_progress'">กำลังดำเนินการ</button>
    ...
    <div class="table-container">
      ...
      <table v-else-if="items.length > 0" class="data-table">
        <thead><tr><th>Tracking ID</th><th>ลูกค้า</th><th>สถานที่</th><th>ที่อยู่</th><th>สถานะ</th><th>ยอดเงิน</th><th>วันที่</th><th>จัดการ</th></tr></thead>
        <tbody>
      ...
      <div v-else class="empty-state"><p>ไม่พบรายการจองคิว</p></div>
    </div>
...
<style scoped>
.queue-view { max-width: 1400px; margin: 0 auto; }
