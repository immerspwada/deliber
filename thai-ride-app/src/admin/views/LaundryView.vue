...
const { isUpdating, updateStatus, exportToCSV, getStatusOptions, formatDate, formatCurrency, getStatusColor, getStatusLabel } = useServiceActions()

const serviceType: ServiceType = 'laundry'
...
const filters = computed<OrderFilters>(() => ({ search: searchQuery.value || undefined, status: statusFilter.value as OrderStatus || undefined }))

async function loadData() {
  const result = await api.getLaundry(filters.value, { page: currentPage.value, limit: pageSize.value })
  items.value = result.data
  total.value = result.total
  totalPages.value = result.totalPages
}

function viewDetail(item: Order) { selectedItem.value = item; showDetailModal.value = true }
function openStatusModal(item: Order) { selectedItem.value = item; showStatusModal.value = true }

async function handleStatusUpdate(orderId: string, newStatus: string) {
  const success = await updateStatus(serviceType, orderId, newStatus)
  if (success) { uiStore.showSuccess('อัพเดทสถานะเรียบร้อย'); showStatusModal.value = false; loadData() }
  else { uiStore.showError('เกิดข้อผิดพลาด') }
}

function handleExport() { exportToCSV(items.value, 'laundry_report'); uiStore.showSuccess('ดาวน์โหลดรายงานเรียบร้อย') }

watch([searchQuery, statusFilter], () => { currentPage.value = 1; loadData() })
watch(currentPage, loadData)
onMounted(() => { uiStore.setBreadcrumbs([{ label: 'Laundry' }]); loadData() })
</script>

<template>
  <div class="laundry-view">
    <div class="page-header">
      <div class="header-left"><h1 class="page-title">Laundry</h1><span class="total-count">{{ total }} รายการ</span></div>
      <button class="btn-export" @click="handleExport"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Export CSV</button>
    </div>
    <div class="filters-bar">
      <div class="search-box"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg><input v-model="searchQuery" type="text" placeholder="ค้นหา..." class="search-input" /></div>
      <select v-model="pageSize" class="filter-select"><option :value="20">20 รายการ</option><option :value="50">50 รายการ</option></select>
    </div>
    <div class="status-tabs">
      ...
      <button class="status-tab" :class="{ active: statusFilter === 'picked_up' }" @click="statusFilter = 'picked_up'">รับแล้ว</button>
      <button class="status-tab" :class="{ active: statusFilter === 'washing' }" @click="statusFilter = 'washing'">กำลังซัก</button>
      <button class="status-tab" :class="{ active: statusFilter === 'ready' }" @click="statusFilter = 'ready'">พร้อมส่ง</button>
    ...
    <div class="table-container">
      <div v-if="api.isLoading.value" class="loading-state"><div v-for="i in 5" :key="i" class="skeleton-row"><div class="skeleton" style="width: 120px;"></div><div class="skeleton" style="width: 100px;"></div><div class="skeleton" style="width: 150px;"></div><div class="skeleton" style="width: 80px;"></div></div></div>
      <table v-else-if="items.length > 0" class="data-table">
        <thead><tr><th>Tracking ID</th><th>ลูกค้า</th><th>ผู้ให้บริการ</th><th>จุดรับ</th><th>จุดส่ง</th><th>สถานะ</th><th>ยอดเงิน</th><th>วันที่</th><th>จัดการ</th></tr></thead>
        <tbody>
      ...
      <div v-else class="empty-state"><p>ไม่พบรายการซักผ้า</p></div>
    </div>
    <div v-if="totalPages > 1" class="pagination"><button class="page-btn" :disabled="currentPage === 1" @click="currentPage--">←</button><span class="page-info">{{ currentPage }} / {{ totalPages }}</span><button class="page-btn" :disabled="currentPage === totalPages" @click="currentPage++">→</button></div>
...
<style scoped>
.laundry-view { max-width: 1400px; margin: 0 auto; }
