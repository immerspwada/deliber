...
async function loadData() {
  const result = await api.getLaundry(filters.value, {
...
function handleExport() {
  exportToCSV(items.value, "laundry_report");
...
onMounted(() => {
  uiStore.setBreadcrumbs([{ label: "Laundry" }]);
  loadData();

  // Setup realtime for laundry_requests
  realtime.subscribe({
    tables: ["laundry_requests"],
    onChange: (table, eventType) => {
      console.log(`[LaundryView] Realtime: ${table} ${eventType}`);
      loadData();
      uiStore.showInfo(
...
<template>
  <div class="laundry-view">
    ...
    <div class="status-tabs">
      ...
      <button
        class="status-tab"
        :class="{ active: statusFilter === 'picked_up' }"
      ...
      <button
        class="status-tab"
        :class="{ active: statusFilter === 'washing' }"
        @click="statusFilter = 'washing'"
      >
        กำลังซัก
      </button>
      <button
