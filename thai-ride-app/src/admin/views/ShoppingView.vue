...
async function loadData() {
  const result = await api.getShopping(filters.value, { page: currentPage.value, limit: pageSize.value })
...
onMounted(() => { uiStore.setBreadcrumbs([{ label: 'Shopping' }]); loadData() })
</script>
  ...
  <div class="shopping-view">
    ...
    <div class="filters-bar">
      <div class="search-box">
        ...
        <input v-model="searchQuery" type="text" placeholder="ค้นหา Tracking ID, ชื่อลูกค้า..." class="search-input" />
      </div>
    ...
    <div class="status-tabs">
      ...
      <button class="status-tab" :class="{ active: statusFilter === 'shopping' }" @click="statusFilter = 'shopping'">กำลังซื้อ</button>
      <button class="status-tab" :class="{ active: statusFilter === 'delivering' }" @click="statusFilter = 'delivering'">กำลังส่ง</button>
      ...
      <table v-else-if="items.length > 0" class="data-table">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id" @click="viewDetail(item)" class="clickable-row">
            ...
            <td><div class="name">{{ item.customer_name || '-' }}</div></td>
            <td><div class="name">{{ item.provider_name || '-' }}</div></td>
            <td><div class="address">{{ item.pickup_address || '-' }}</div></td>
            <td><div class="address">{{ item.dropoff_address || '-' }}</div></td>
            <td>
              <span 
                class="status-badge" 
                :style="{ color: getStatusColor(item.status), background: getStatusColor(item.status) + '20' }"
              >
            ...
            <td @click.stop>
              <button class="action-btn" @click="openStatusModal(item)" title="เปลี่ยนสถานะ">
...
<style scoped>
.shopping-view { max-width: 1400px; margin: 0 auto; }
...
.btn-export { display: flex; align-items: center; gap: 8px; padding: 10px 16px; background: #fff; border: 1px solid #E5E7EB; border-radius: 10px; font-size: 14px; font-weight: 500; color: #374151; cursor: pointer; transition: all 0.2s; }
...
.status-tab { padding: 8px 16px; background: #fff; border: 1px solid #E5E7EB; border-radius: 8px; font-size: 14px; font-weight: 500; color: #6B7280; cursor: pointer; transition: all 0.2s; }
...
.data-table th { padding: 14px 16px; text-align: left; font-size: 12px; font-weight: 600; color: #6B7280; background: #F9FAFB; border-bottom: 1px solid #E5E7EB; text-transform: uppercase; }
...
.address { font-size: 13px; color: #6B7280; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.action-btn { width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; background: none; border: 1px solid #E5E7EB; border-radius: 6px; cursor: pointer; color: #6B7280; transition: all 0.2s; }
