...
| **F254**  | Quick Reorder System                  | `useQuickReorder.ts`, `QuickReorderCard.vue`, `CustomerHomeView.vue`                        | `ride_requests`, `delivery_requests`, `shopping_requests`, `reorder_analytics`                            | 170       |

### Service Bundles (F167)

| รหัส     | ฟีเจอร์                | Composable/Component                                  | ตาราง DB                              | Migration |
| -------- | ---------------------- | ----------------------------------------------------- | ------------------------------------- | --------- |
| **F167** | Service Bundles System | `useServiceBundles.ts`, `AdminServiceBundlesView.vue` | `service_bundles`, `bundle_templates` | 167       |
...
| รหัส     | ฟีเจอร์                | Composable/Component                                                   | ตาราง DB                                                                                                                      | Migration |
| -------- | ---------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------- |
| **F03a** | Delivery Proof Photo   | `useProvider.ts` → `uploadDeliveryProof()`, `DeliveryProofCapture.vue` | `delivery_requests` (delivery*proof_photo, delivery_proof_lat, delivery_proof_lng, delivery_proof_timestamp, pickup_proof*\*) | 051       |
...
reorder_analytics       → F254 (Quick Reorder) - user_id, service_type, original_request_id, new_request_id, reorder_method, reordered_at
```
...
| `get_reorderable_items()`                | Get list of reorderable items for user                   | F254             |
| `quick_reorder_ride()`                   | Create new ride request from previous ride               | F254             |
| `quick_reorder_delivery()`               | Create new delivery request from previous delivery       | F254             |
